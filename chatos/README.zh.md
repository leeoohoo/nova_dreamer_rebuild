...
- **任务追踪**：通过 `mcp_subagent_router_run_sub_agent` 运行子代理，配合 `mcp_task_manager_*` 记录任务与进展。
...
- 引擎源码与 ChatOS 同仓，目录为 `./packages/aide`（无需外部 embed root）。
...
## MCP 工具（主 vs 子）
- 主代理白名单：`get_current_time`、`mcp_project_files_*`、`mcp_subagent_router_*`、`mcp_task_manager_*`（默认不允许 shell；其它 MCP 默认仅子代理可用）。
...
```
packages/aide/src/           # CLI Core、聊天循环、prompt、MCP runtime
packages/aide/subagents/     # 子代理管理、marketplace、插件（python / spring-boot / frontend-react）
packages/aide/mcp_servers/   # Shell（含会话工具）等 MCP 服务器
